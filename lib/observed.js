 ...
 * @param {Function} blacklistFn Function to call to determine if path is
 *                               on a blacklist. Passed full path to change.
 * @param {Observable} [parent] (internal use)
 * @param {String} [prefix] (internal use)
 * @return {EventEmitter}
 */

function Observable (subject, blacklistFn, parent, prefix) {
  if ('object' != typeof subject)
    throw new TypeError('object expected. got: ' + typeof subject);

  if (!(this instanceof Observable))
    return new Observable(subject, blacklistFn, parent, prefix);

  debug('new', subject, !!parent, prefix);

  this._blacklistFn = blacklistFn;
...
Observable.prototype._bind = function (subject, parent, prefix) {
  ...
  this.onchange = onchange(parent || this, prefix).bind(this);
  if (Array.isArray(this.subject)) {
      Array.observe(this.subject, this.onchange);
  } else {
...
Observable.prototype._walk = function (parent, prefix) {
  ...
  var blacklistFn = this._blacklistFn;
  ...
  Object.keys(object).forEach(function (name) {
    ...
    if (!blacklistFn || !blacklistFn(path)) {
      new Observable(value, blacklistFn, parent, path);
    }
  });
}
...
Observable.prototype._remove = function (subject) {
  ...
  this.observers = this.observers.filter(function (observer) {
    if (subject == observer.subject) {
      if (Array.isArray(observer.subject)) {
          Array.unobserve(observer.subject, observer.onchange);
      } else {
          Object.unobserve(observer.subject, observer.onchange);
      }
      return false;
...
function onchange (parent, prefix) {
  return function (ary) {
    ...
    ary.forEach(function (change) {
      ...
      if (that._blacklistFn && that._blacklistFn(path)) {
          return;
      }

      if ('add' == type && null != value && 'object' == typeof value) {
        new Observable(value, that._blacklistFn, parent, path);
